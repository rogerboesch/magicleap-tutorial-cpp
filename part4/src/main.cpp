...
int main() {
	...
	// If engine stops, end main loop, also support of Pause
	while (game.GetState() != rb::ENGINE_STOPPED) {
		...
		if (frame_result == MLResult_Ok) {
			...
			for (int camera = 0; camera < 2; ++camera) {
				...
				// Part 4: Get camera position and pass to engine
				glm::vec3 cameraPosition = rb_camera_position(current_camera);
